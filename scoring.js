 ...
 *   small_volume_ratio  小票成交占比，百分比形式（如 16 表示 16%）— 无真实数据时**不要**设此字段
 ...
 *   market_concentration 市场集中度，0~1小数（无真实数据时**不要**设此字段）
 *   fund_avg_1d/1w/1m   观察池基金近1日/1周/1月平均收益，百分比形式（只统计有效数据）
 *   fund_rel_zz2000     基金相对中证2000超额，百分比形式
 *   fund_cont_days      连续修复天数，整数
 *   hard_risk_*         硬风控状态：'ok' / 'watch' / 'trigger'
 *
 * 【关键规则】
 *   1. 无真实数据的指标**不要**赋值，直接不设字段名。评分引擎会自动跳过。
 *   2. 禁止用微盘成交占比 × 倍数 假冒小票成交占比 — 这会让拥挤评分严重失真。
 *   3. 基金 null 数据不参与平均收益计算。
 *
 * 数据类型标注（用于评分拆解表展示）：
