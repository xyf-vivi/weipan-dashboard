 ...
 *
 * 核心改动（基于用户反馈 v4）：
 * 1. 删除 allA_median_chg（微盘近5日代理全A中位数），不再纳入评分
 * 2. 删除 wp_ma243_pos（回撤代理243日均线），不再纳入评分
 * 3. wp_slope20 权重从 8/4 降为 3/1，改名为"趋势方向代理"
 * 4. 硬风控改为"接入进度"模式，不输出 ok/trigger 结论
 * 5. 利率约束(rate)和波动拥挤(volatility)移出硬风控主评分，
 *    只在"接入进度"中展示状态，不参与分数计算
 * 6. MODULE_INFO / calcConfidence 同步更新
 */

const ScoringEngine = {
  // === 硬风控指标定义（仅作接入进度展示，不参与分数计算）===
  HARD_RISK: {
    volatility:      { label: '波动拥挤',       dataType: 'wait',    defaultStatus: 'wait' },
    rate:           { label: '利率约束',       dataType: 'wait',    defaultStatus: 'wait' },
    liquidity:      { label: '流动性踩踏',     dataType: 'auto',    defaultStatus: 'ok'   },
    homogenization: { label: '产品同质化',     dataType: 'lowfreq', defaultStatus: 'wait' }
  },

  // === 已接入的项才参与硬风控分数计算 ===
  HARD_RISK_SCORED: ['liquidity', 'homogenization'],

  RULES: {
    // === 硬风控（权重：闸门，仅 liquidity + homogenization 计分）===
    hardRisk: {
      max: 20,
      evaluate(data) {
        let triggerCount = 0, okCount = 0, waitCount = 0;
        const itemStatus = {};

        ['hard_risk_volatility', 'hard_risk_rate', 'hard_risk_liquidity', 'hard_risk_homogenization'].forEach(k => {
          const s = data[k] || 'wait';
          itemStatus[k.replace('hard_risk_','')] = s;
          if (s === 'trigger') triggerCount++;
          else if (s === 'ok') okCount++;
          else if (s === 'wait') waitCount++;
        });

        // 触发 → 直接回避区
        if (triggerCount > 0) return { status: 'trigger', score: 0, max: 20, itemStatus };

        // 只给已确认 ok 的项加分（liquidity + homogenization）
        let score = 0;
        if (data.hard_risk_liquidity === 'ok') score += 10;
        if (data.hard_risk_homogenization === 'ok') score += 5; // 低频，接入后加分
        // watch 项（rate）给少量分
        if (data.hard_risk_rate === 'watch') score += 3;

        const status = waitCount >= 3 ? 'wait' : (okCount > 0 ? 'ok' : 'watch');
        return { status, score: Math.min(20, score), max: 20,
                 detail: { triggerCount, okCount, waitCount },
                 itemStatus };
      }
    },
    ...
    styleDirection: {
      ...
      evaluate(data) {
        ...
        // 20日趋势方向代理（涨跌幅/20，非真实回归斜率）
        const slope = data.wp_slope20;
        if (slope !== undefined) {
          if (slope > 0)  { score += 3; details.slope = '趋势方向向上（代理）'; }
          else if (slope > -0.5) { score += 1; details.slope = '趋势方向接近零（代理）'; }
          else { details.slope = '趋势方向向下（代理）'; }
        }

        // ⚠️ 243日均线：无真实数据，不纳入评分
        details.ma243 = '无真实数据，不纳入评分';
    ...
    crowdingLiquidity: {
      ...
      evaluate(data) {
        ...
        if (tp !== undefined && tp !== null) {
          if (tp > 0 && tp < 0.8) { score += 5; details.turnover = '分位适中（近28日样本）'; }
          else if (tp >= 0.8 && tp < 0.95) { score += 2; details.turnover = '分位偏高（近28日样本）'; }
          else { details.turnover = '分位极端（近28日样本）'; }
        } else {
    ...
    // === 量化友好度（20分）===
    // ⚠️ 全A中位数涨跌幅：无真实数据，不纳入今日评分；只用上涨家数占比
    quantFriendliness: {
      max: 20,
      evaluate(data) {
        let score = 0;
        ...
        if (upR !== undefined && upR !== null) {
          if (upR > 0.6)  { score += 12; details.upRatio = '占比>60%，赚钱效应扩散'; }
          else if (upR > 0.5) { score += 7; details.upRatio = '占比50-60%，中性'; }
          else { score += 3; details.upRatio = '占比<50%，偏低'; }
        } else {
          details.upRatio = '待接入';
        }

        // 全A中位数涨跌幅：无真实数据，不纳入今日评分
        details.median = '无真实数据，不纳入今日评分（不要用微盘代理）';
  ...
  MODULE_INFO: {
    hardRisk: {
      label: '硬风控接入进度',
      dataType: 'mixed',
      meaning: '哪些风险项已接入、哪些待接入',
      included: '流动性踩踏（已接入）、产品同质化（低频）',
      excluded: '波动拥挤、利率约束（待接入，不参与今日评分）'
    },
    styleDirection: {
      label: '风格方向',
      dataType: 'partial',
      meaning: '微盘有没有重新占优',
      included: '中证2000 vs 科创50相对强弱（真实）、20日趋势方向（代理，低权重）',
      excluded: '243日均线（待接入）、微盘/茅指数相对净值（待接入）'
    },
    crowdingLiquidity: {
      label: '拥挤与流动性',
      dataType: 'proxy',
      meaning: '资金是否健康回流',
      included: '微盘成交占比（真实）、换手率分位（样本内28日，精度有限）',
      excluded: '小票成交占比（待接入）、相对换手率（待接入）'
    },
    quantFriendliness: {
      label: '量化友好度',
      dataType: 'partial',
      meaning: '市场是否适合分散量化赚钱',
      included: '上涨家数占比（真实）',
      excluded: '全A中位数（无数据）、市场集中度、横截面分化度（待接入）'
    },
    productVerification: {
      label: '产品验证',
      dataType: 'auto',
      meaning: '基金净值是否确认',
      included: '微盘暴露型+分散量化型基金均值（真实，有效样本有限）',
      excluded: '风格对照型不参与产品验证评分'
    }
  },
  ...
  calculate(data) {
    ...
    if (hardRisk.status === 'trigger') {
      return {
        ...
        hardRiskProgress: this.getHardRiskProgress(data)
      };
    }
  ...
  // 硬风控接入进度（新增，供前端展示）
  getHardRiskProgress(data) {
    const items = [
      { key: 'volatility',      label: '波动拥挤模型',     status: data.hard_risk_volatility || 'wait',  note: '待研究公式，不参与日频判断' },
      { key: 'rate',           label: '利率约束',         status: data.hard_risk_rate || 'wait',        note: '10年国债已接入（' + (data.bond_10y || '—') + '%），但风控阈值待定义，仅宏观观察' },
      { key: 'liquidity',      label: '流动性踩踏',       status: data.hard_risk_liquidity || 'wait',    note: '已接入（下跌家数）' },
      { key: 'homogenization', label: '产品同质化',       status: data.hard_risk_homogenization || 'wait', note: '低频观察，不参与日频评分' }
    ];
    return items;
  },
  ...
  calcConfidence(data) {
    ...
    const realChecks = [
      ...
      // 硬风控：只有 liquidity 是真实接入的
      { has: data.hard_risk_liquidity !== undefined && data.hard_risk_liquidity !== 'wait', weight: 5 }
    ];
    ...
    const proxyChecks = [
      ...
      { has: data.wp_slope20 !== undefined,      weight: 5,  label: '20日趋势方向（代理）' }
    ];
  ...
  getLevel(score, hardRiskStatus) {
    ...
    if (hardRiskStatus === 'wait' && score > 65)
      return { label: '观察区', color: '#ca8a04', desc: '硬风控多项待接入，最高只能观察区。' };
