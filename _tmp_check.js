...
function renderHardRiskProgress(progressItems) {
  ...
  if (!container) return;
  if (!progressItems || !Array.isArray(progressItems)) { container.innerHTML = '<div style="font-size:12px;color:var(--text-muted)">数据不足</div>'; return; }
  const iconMap = { ok:'✅', watch:'⚠️', trigger:'❌', wait:'⏳' };
  container.innerHTML = progressItems.map(item => {
    const icon = iconMap[item.status] || '⏳';
    let noteHtml = '';
    if (item.key === 'rate') {
      const bond = (AUTO_DATA && AUTO_DATA.bond_10y != null) ? AUTO_DATA.bond_10y.toFixed(2) + '%' : '—';
      noteHtml = `<span style="font-size:10px;color:var(--text-muted);margin-left:6px">10年国债${bond}，风控阈值待定义，仅宏观观察</span>`;
    } else {
      noteHtml = `<span style="font-size:10px;color:var(--text-muted);margin-left:6px">${item.note || ''}</span>`;
    }
...
function renderStyleFold(styleTier) {
  ...
  const html = `
    <div style="margin-top:20px;padding:12px;background:rgba(59,130,246,.05);border-radius:8px;border:1px dashed var(--accent-blue)">
      <div class="pending-fold-toggle" onclick="toggleFold('${foldId}')">
        📊 ${styleTier.title} ▼
      </div>
      <div class="pending-content" id="${foldId}" style="display:none">
        <div style="font-size:11px;color:var(--text-muted);margin-bottom:8px;padding:6px 8px;background:var(--bg-hover);border-radius:6px">${styleTier.desc}</div>
        <table class="fund-table">
          <thead><tr><th>基金</th><th>角色</th><th>近1日</th><th>近1周</th><th>近1月</th><th>数据状态</th><th>观察结论</th></tr></thead>
          <tbody>
            ${styleTier.funds.map(f => {
              const ds = f.dayChange !== null && f.dayChange !== undefined ? { text: '完整', cls: 'tag-green' } : { text: '待接入', cls: 'tag-gray' };
              const role = styleTier.roles[f.name] || '—';
              return `<tr style="opacity:0.7">
                ...
                <td style="color:${pctColor(f.month1)}">${fmtPct(f.month1)}</td>
                <td><span class="tag ${ds.cls}">${ds.text}</span></td>
                <td style="font-size:11px;color:var(--text-muted)">非微盘确认信号</td>
              </tr>`;
            }).join('')}
          </tbody>
        </table>
...
function renderChecklists(data) {
  ...
  if (zzArr.length >= 4 && kcArr.length >= 4) {
    ...
    for (let i = 1; i < zzArr.length; i++) {
      ...
      const isBeat = zzRet > kcRet;
      if (isBeat) beatCount++;
      // 假设日期从6月10日开始（根据实际数据调整）
      const date = `6月${9+i}日`;
  ...
  const confirmItems = [
    ...
    { text: '连续3个交易日，小票跑赢科技', status: canCalc3d ? (beatCount >= 3 ? 'check' : 'cross') : 'wait',
      note: canCalc3d ? `近3日详情：\n${beatDetails.join('\n')}\n\n合计跑赢${beatCount}次` : '日K线数据不足' },
    // #12: 改用已有微盘成交占比判断连续回升
